...
1. The prompts used to generate the media that will be evaluated. Models are matched against each other per prompt.
...
1. Images, videos, or audio files generated by your model.
2. Each media file must be paired with a prompt from the benchmark's registered prompt set. You can supply the same prompt multiple times.
