...
class FaucetApi:
    ...
    def leaderboard_faucet_replicate_models_model_owner_model_name_get(
        ...
        The returned parameters are the model's input schema as the provider reports it, so each one  can be bound to a literal value or to a benchmark prompt segment. The prompt input is  included and takes a segment binding only, because a literal there would replace every  benchmark prompt with the same text; it falls back to the prompt's default text segment when  a faucet leaves it unbound. Every other input, an image input included, is sent only when  the faucet binds it — bind one to an asset segment to send a reference image.  num_outputs is excluded: the system supplies it per run and no binding on it is  accepted.
