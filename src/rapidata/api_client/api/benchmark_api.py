...
class BenchmarkApi:
    ...
    def benchmark_benchmark_id_matchups_query_get(
        ...
        group_by: Annotated[Optional[RatingGrouping], Field(description="What a compared image stands for. Per image, every matchup is returned. Per participant, a  matchup between two images of one participant is a self-matchup and leaves the page.")] = None,
        page: Annotated[Optional[StrictInt], Field(description="The 1-based page index.")] = None,
        page_size: Annotated[Optional[StrictInt], Field(description="The number of items per page.")] = None,
        sort: Annotated[Optional[List[StrictStr]], Field(description="Sort fields. Prefix with - for descending order (e.g. -created_at).")] = None,
        country: Annotated[Optional[AudienceAudienceIdJobsGetJobIdParameter], Field(description="Filter by country.")] = None,
        ...
        An item is one image pair as one leaderboard saw it, so a pair compared on several leaderboards  comes back once per leaderboard and the boards' verdicts on it can be read side by side.  Ordering (sigma by default) ranks the matchups themselves — sigma being how far apart the two  images' records over all of their comparisons on the prompt sit, in standard errors of their  logit gap — where the prompt-ratings query ranks prompts by their most decisive matchup. Filters  use that query's field set, applied to the votes a matchup is tallied from, plus four fields  that select matchups by their outcome once tallied: WinnerParticipantId and  LooserParticipantId pick the side that won the whole matchup (a tied matchup matches  neither), while Responses and Extremeness take range comparisons and select on  how much evidence the matchup carries. The four are only accepted as top-level and  leaves. Matchups of leaderboards excluded from the benchmark overall are never returned.  Responses and Extremeness together express the fair-coin sign test on the  split: extremeness[gte]=1.96 with responses[gte]=5 keeps the matchups a coin  flip would reproduce less than 5% of the time. The response floor is not redundant — the most  lopsided three-response outcome is a 3–0 sweep at p = 0.125, and a 4–0 clears 1.96 standard  deviations at p = 0.0625.  WinnerSigma answers a different question: not how decisive this matchup's own split is,  but whether the two images' pooled records agree with it. It is sigma signed from the  winning side, so winner_sigma[gt]=0 keeps the matchups the records confirm and  winner_sigma[lt]=0 keeps the upsets. Ordering by sigma cannot separate the two —  a wide gap in either direction ranks the same — which is what sorting by -winner_sigma  fixes.  sides holds exactly two images, the one that took more responses first, and their  responses sum to the matchup's. Each side carries its theta — the pooled per-image  record sigma is derived from. extremeness and the winning side are always read off the  raw counts, and sigma/theta are always weighted by annotator reliability;  useWeightedScoring changes only each side's wins.
        ...
        :param group_by: What a compared image stands for. Per image, every matchup is returned. Per participant, a  matchup between two images of one participant is a self-matchup and leaves the page.
