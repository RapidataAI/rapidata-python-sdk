...
class ValidationSetApi:
            ...
            if isinstance(_param_val, dict):
                for _k, _v in _param_val.items():
                    ...
                    if isinstance(_v, list):
                        ...
                        # params: field[in]=a&field[in]=b.
                        for _item in _v:
                            _query_params.append(('type[' + _k + ']', _item))
                    else:
    ...
    def validation_sets_compatible_get(
        ...
        Matching is strict: a validation set is only returned when its asset type, modality, and prompt type  equal the provided parameters exactly — the same constraint enforced when actually adding a rapid.  Uninitialized validation sets (no modality yet — they adopt the classifiers of the first rapid added  to them) are compatible with any rapid and are always returned, even while still empty.  Results cover the caller's own and public validation sets; callers allowed to query across  organizations see compatible sets from every organization.  This differs from /validation-set/recommended, which uses loose overlap matching and may return  sets that then reject the rapid. Returns an empty array when no compatible set exists.
            ...
            if isinstance(_param_val, dict):
                for _k, _v in _param_val.items():
                    ...
                    if isinstance(_v, list):
                        ...
                        # params: field[in]=a&field[in]=b.
                        for _item in _v:
                            _query_params.append(('asset_type[' + _k + ']', _item))
                    else:
            ...
            if isinstance(_param_val, dict):
                for _k, _v in _param_val.items():
                    ...
                    if isinstance(_v, list):
                        ...
                        # params: field[in]=a&field[in]=b.
                        for _item in _v:
                            _query_params.append(('modality[' + _k + ']', _item))
                    else:
            ...
            if isinstance(_param_val, dict):
                for _k, _v in _param_val.items():
                    ...
                    if isinstance(_v, list):
                        ...
                        # params: field[in]=a&field[in]=b.
                        for _item in _v:
                            _query_params.append(('prompt_type[' + _k + ']', _item))
                    else:
