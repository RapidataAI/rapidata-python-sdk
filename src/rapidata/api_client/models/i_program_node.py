...
class IProgramNode(LazyValidatedModel):
    ...
    IProgramNode
    """
    # data type: IProgramNodeActionNode
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = IProgramNode.model_construct()
        error_messages = []
        match = 0
        # validate data type: IProgramNodeActionNode
        if not isinstance(v, IProgramNodeActionNode):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IProgramNodeActionNode`")
        else:
            match += 1
        # validate data type: IProgramNodeCasesNode
        if not isinstance(v, IProgramNodeCasesNode):
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into IProgramNodeActionNode
        try:
            instance.actual_instance = IProgramNodeActionNode.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into IProgramNodeCasesNode
        try:
            instance.actual_instance = IProgramNodeCasesNode.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
