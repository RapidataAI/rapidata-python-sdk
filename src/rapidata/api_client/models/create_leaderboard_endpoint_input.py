...
class CreateLeaderboardEndpointInput(LazyValidatedModel):
    ...
    skip_initial_run: Optional[StrictBool] = Field(default=None, description="Set to true to skip the initial standings run. A newly added model is still  compared against the whole existing field and boost runs still work — there is just no  run collecting the first responses. On the benchmark's first leaderboard, participants  that already have result images are submitted into the field so those paths have a field  to work with; on a benchmark that already has a leaderboard they are in it already and  are left untouched. Defaults to false, which queues the run as before.", alias="skipInitialRun")
