...
class RESTClientObject:
        ...
        for attempt in range(_TRANSIENT_RETRY_MAX_ATTEMPTS + 1):
            try:
                r = self._send_request(
                    self.session, method, url, headers, body, post_params, timeout
                )
            ...
            if r.status_code in _RETRYABLE_STATUS_CODES and attempt < _TRANSIENT_RETRY_MAX_ATTEMPTS:
                ...
                _logger.warning(
                    ...
                    url,
                    attempt + 1,
                    _TRANSIENT_RETRY_MAX_ATTEMPTS + 1,
                    r.status_code,
                    delay,
                )
                time.sleep(delay)
    ...
    def _send_request(self, session, method, url, headers, body, post_params, timeout):
        ...
        if method in ["POST", "PUT", "PATCH", "OPTIONS", "DELETE"]:
            return self._send_body_request(
                session, method, url, headers, body, post_params, timeout
            )
    ...
    def _send_body_request(session, method, url, headers, body, post_params, timeout):
        ...
        if not content_type or re.search("json", content_type, re.IGNORECASE):
            ...
            return session.request(
                method, url, content=request_body, timeout=timeout, headers=headers
            )
        ...
        if content_type == "multipart/form-data":
            ...
            return session.request(
                method, url, files=files, data=data, timeout=timeout, headers=headers
            )
    ...
    def _handle_http_error(self, error, method, url, attempt):
        ...
        """
        if isinstance(error, ConnectError) and self._is_certificate_validation_error(
            error
        ):
            raise ApiException(
                status=0,
    ...
    def _get_session_defaults(self):
        ...
        if self.configuration.retries is not None:
            transport = httpx.HTTPTransport(
                retries=self.configuration.retries, limits=limits
            )
