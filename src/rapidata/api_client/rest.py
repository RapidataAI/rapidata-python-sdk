...
# httpx2 disables timeouts entirely when one is passed explicitly as `None`, so every
...
class RESTClientObject:
    ...
    def request(
        ...
        raise ApiException(status=0, reason="All retry attempts exhausted")
    ...
    @staticmethod
    def _retry_after_from_response(response: httpx2.Response) -> Optional[float]:
    ...
    def _build_timeout(_request_timeout):
        ...
        Falls back to USE_CLIENT_DEFAULT rather than None, because httpx2 reads an
    ...
    def _get_session_defaults(self):
        ...
        if self.configuration.proxy:
            ...
            if self.configuration.proxy_headers:
                # httpx2 doesn't have a dedicated `proxy_headers` kwarg the way
        ...
        if self.configuration.retries is not None:
            transport = httpx2.HTTPTransport(retries=self.configuration.retries, limits=limits)
            client_kwargs["transport"] = transport

        return client_kwargs

    @staticmethod
    def _is_retryable_error(error: httpx2.HTTPError) -> bool:
