...
def _default_enable_otlp() -> bool:
    ...
    return os.environ.get("RAPIDATA_DISABLE_OTLP", "0").lower() not in (
        "1",
        "true",
        "yes",
    )
...
class LoggingConfig(BaseModel):
    ...
    Attributes:
        ...
        enable_otlp (bool): Whether to enable OpenTelemetry trace logs. Defaults to True;
            set RAPIDATA_DISABLE_OTLP=1 to default it to False. Export only starts once a
            RapidataClient has been created, so SDK components driven without one (for
            example with mocked services in a test) never export.
